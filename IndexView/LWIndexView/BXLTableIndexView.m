...
#import "BXLTableIndexIndicateView.h"
...
CGFloat const kIndexViewWidth = 15.0;

@interface BXLTableIndexView()<CAAnimationDelegate,BXLTableViewScrollObserverDelegate>{
    BOOL _isTouch;
}
...
@property (nonatomic, strong) NSMutableArray <UILabel
*>*dotLabelsArray;
@property (nonatomic, strong) BXLTableIndexIndicateView *indicateView;
@property (nonatomic, strong) BXLTableViewScrollObserver *tableViewOberrver;
@end

@implementation BXLTableIndexView
...
- (void)_gestureAction:(UIGestureRecognizer *)gesture {
    ...
    if (locatedLabel
        ...
        if ([_delegate respondsToSelector:@selector(BXLTableIndexViewDidClickIndex:)]) {
            [_delegate BXLTableIndexViewDidClickIndex:selectedString];
        }
    }
...
- (void)showIndicateViewInAnchorPoint:(CGPoint) point {
    self.indicateView.backgroundColor = self.configuration.normalTextColor;
...
- (BXLTableIndexIndicateView *)indicateView {
    if (!_indicateView) {
        _indicateView = [[BXLTableIndexIndicateView alloc] initWithFrame:CGRectZero];
    }
